    ...
    public function getConditionContent()
    {
        ...
        if ($condition !== false) {
            if ($this->getConfigData('is_html')) {
                $conditionData = $this->escapeHtml($condition);
            } else {
                $params = ['allowableTags' => '<img>,<a>'];
                $condition = htmlspecialchars_decode($condition);
                $condition = $this->stripTags($condition, $params);
                $conditionData = preg_replace("/[\r\n]+/", "\n", $condition);
                $conditionData = nl2br($conditionData);
            }
        }

        if ($privacy !== false) {
            if ($this->getConfigData('is_html')) {
                $privacyData = $this->escapeHtml($privacy);
            } else {
                $params = ['allowableTags' => '<img>,<a>'];
                $privacy = htmlspecialchars_decode($privacy);
                $privacy = $this->stripTags($privacy, $params);
                $privacyData = preg_replace("/[\r\n]+/", "\n", $privacy);
                $privacyData = nl2br($privacyData);
